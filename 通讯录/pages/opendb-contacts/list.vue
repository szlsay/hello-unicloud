...
<script>
	...
	export default {
		data() {
			return {
				collectionList: [db.collection('opendb-contacts').field(
					'username,gender,mobile,email,comment,nation,addressCity').getTemp(), db.collection(
					'opendb-nation-china').field('_id, name').getTemp()],
				loadMore: {
					contentdown: '',
					contentrefresh: '',
					contentnomore: ''
